...
with open(prefix + "GSA_d.txt") as file:
...
name = str(n) + "_" +  str(accuracy_factor) + "_"+ str(blocksize) + "_"+ str(N) + "_"+ str(c)
...
ratio = [(sz[i]/sz[i-1]) for i in range(1,len(sz))]
...
plt.tight_layout()
plt.savefig(prefix + "plots/"+ name + ".png", dpi = 300)

with open(prefix + 'GSA_d.txt','r') as firstfile, open(prefix + name + '.txt','w') as secondfile:
