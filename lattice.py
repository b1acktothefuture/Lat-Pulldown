...
class BKZs:
    ...
    def __call__(self, block_size):
        ...
        while True:
            clean = self.bkz_loop(block_size, 0, self.A.nrows, enum_vecs)
            if clean:
                break
    ...
    def bkz_loop(self, block_size, min_row, max_row, enum_vecs):
        ...
        for kappa in range(min_row, max_row-1):
            ...
            clean &= self.svp_reduction(kappa, bs, enum_vecs)
        return clean

    def svp_reduction(self, kappa, block_size, enum_vecs):
        ...
        max_dist, solution = Enumeration(self.m).enumerate(
            kappa, kappa + block_size, max_dist, expo, pruning=None)[0]
        # use different pruning stratergies
        # print(solution)

        if max_dist >= delta_max_dist * (1 << expo):
            return clean
        # How does this relate to termination??
        ...
        if nonzero_vectors == 1:
            ...
            self.lll_obj.size_reduction(
                kappa, kappa + first_nonzero_vector + 1)
            # print(self.m.B[kappa])
            # get the row at [kappa] index, check if already size reduced
            ...
            with self.m.row_ops(d, d+1):
                for i in range(block_size):
                    self.m.row_addmul(d, kappa + i, solution[i]) # last vector is the SVP result

            self.m.move_row(d, kappa)
            self.lll_obj(kappa, kappa, kappa + block_size + 1) # lll reduction, how to get only size reduced vector
            self.m.move_row(kappa + block_size, d)

            self.m.remove_last_row()
            # print(self.m.B[kappa])
            # get the row at [kappa] index, the block is lll reduced, so size reduced
        vec = []
        for i in range(self.m.B.ncols):
            vec.append(self.m.B[kappa][i])
        enum_vecs.add(tuple(vec))
...
def test_bkz():
    ...
    after = []
    for vecs in mat:
        before.append(l2_norm(vecs,None))
    BKZs(mat)(10)
    for vecs in mat:
        after.append(l2_norm(vecs,None))
